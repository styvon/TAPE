...
TAPEtest_profiling <- function(null_object, genfile, samplefile, outfile, genfile_format="bfile", thresh_maf=0, thresh_mac=0, snpbatch_size=20000, espa_nt=9999, espa_range=c(-20,20), 
                     bgi_file="1",snplist_file="", verbose=FALSE){
  if(verbose){
    cat("Starting TAPEtest profiling...\n")
  }
  # ==== validate null model ======
  if(!(class(null_object) %in% c("glmmkin", "glmmai","tape_null"))){
    stop("Object class not supported")
...
TAPEtest_ioonly <- function(null_object, genfile, samplefile, outfile, genfile_format="bfile", thresh_maf=0, thresh_mac=0, snpbatch_size=20000, espa_nt=9999, espa_range=c(-20,20), 
                     bgi_file="1",snplist_file="", verbose=FALSE,n_memchunk=1){
  outfile = paste0(outfile,"_ioonly")
  if(verbose){
    cat("Starting TAPEtest...\n")
  }
  # ==== validate null model ======
